...
class SymbolTradingManager {
      ...
      if (progressPercent >= trailingStopLoss.progressThreshold) {
        ...
        const newStopLoss = this.zetaWrapper.roundToTickSize(
          isShort
            ...
            : entryPrice +
                totalDistanceToTP * trailingStopLoss.stopLossDistance,
        );
  ...
  generatePositionId(position) {
    return `${this.symbol}-${position.size > 0 ? "LONG" : "SHORT"}-${position.costOfTrades}`;
  }
...
async function setupPriorityFees() {
  try {
    ...
    const recentFees = await fetchSolanaPriorityFee(priorityFeesConnection, 150, []);
    ...
    currentPriorityFee = Math.floor(
      initialFee * priorityFeeMultiplier,
    );
